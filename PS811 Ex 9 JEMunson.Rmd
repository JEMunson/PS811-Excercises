...
# ms: i think what is going on is that the plyr package also has a here() command, and the plyr package was probably loaded along with magrittr and tidyverse...
# ms: one way to get around this is doing here::here(), which means that you want to use the here command within the here package...
```
...
# ms: you need to install.packages("stargazer") then run library(stargazer) 
```
